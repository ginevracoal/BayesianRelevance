...
python3 attack_torchvision_networks.py --model_name=$MODEL_NAME  --dataset_name=$DATASET_NAME --bayesian=$BAYESIAN --inference=$INFERENCE --train=$TRAIN --attack=$ATTACK --iters=$ITERS --attack_method=$ATTACK_METHOD --n_samples=$N_SAMPLES --device=$DEVICE &> $OUT
